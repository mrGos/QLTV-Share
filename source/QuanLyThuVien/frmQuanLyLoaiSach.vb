            ...
            Select Case MsgBox("Bạn có thực sự muốn xóa loại sách có mã: " + txtMaLoai.Text, MsgBoxStyle.YesNo, "Information")
                Case MsgBoxResult.Yes
                    Try
                        ...
                        '1. Delete from DB
                        Dim result As Result
                        result = lsBUS.delete(txtMaLoai.Text)
                        If (result.FlagResult = True) Then
                        ...
                        Else
                            MessageBox.Show("Xóa Loại sách không thành công. Kiểm tra ràng buộc với sách", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                            System.Console.WriteLine(result.SystemMessage)
                        End If
                    Catch ex As Exception
                        Console.WriteLine(ex.StackTrace)
